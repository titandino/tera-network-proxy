  ...
  loadSettings() {
    ...
    try {
      ...
      if(e.toString().includes('at position 0')) {
          this.error('WARNING: You closed proxy improperly the last time you were using it!');
          this.error(`WARNING: This caused the settings for module "${this.name}" to become corrupted!`);
          this.error('WARNING: The module will load default settings now, so adjust them according to your needs.');
          this.error('WARNING: Please remember to close proxy properly: first close the game, then close proxy using the X button!');
          this.error('WARNING: Do not shut down your computer while proxy is running!');

          this.settings = this.migrateSettings(null, this.settingsVersion);
          this.saveSettings();
          return;
      } else {
          this.error(`ERROR: Invalid settings format for module "${this.name}"!`);
          this.error('ERROR: This means that you broke it when manually editing it.');
          this.error('ERROR: Please fix the settings file manually or delete it so proxy can generate default settings for you.');
          this.error('------------------------------------------');
